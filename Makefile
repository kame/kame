...
# only for developers
bsdi3:
	(cd ${.CURDIR}; set CVSROOT=cvs.kame.net:/cvsroot/kame-local; export CVSROOT; cvs -d cvs.kame.net:/cvsroot/kame-local co -r stable_200001 -d bsdi3 -P kame/bsdi3)

bsdi4:
	(cd ${.CURDIR}; set CVSROOT=cvs.kame.net:/cvsroot/kame-local; export CVSROOT; cvs -d cvs.kame.net:/cvsroot/kame-local co -r stable_200001 -d bsdi4 -P kame/bsdi4)
...
update-doc:
	(cd ${.CURDIR}; cvs update -d -P -r stable_200001 ${DOCS} ${TOOLS})
update-plat:
	(cd ${.CURDIR}; \
	for i in kame ${TARGET}; do \
		if test -d $$i; then \
			(cd $$i; cvs update -d -P -r stable_200001); \
		fi \
	done)
...
tree:
	...
	if test $(TARGET) = bsdi3  -o $(TARGET) = bsdi4; then \
		...
		cvs update -d -P -r stable_200001 $(TARGET); \
	fi; \
	cvs update -d -P -r stable_200001 kame)
