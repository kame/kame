...
int	attrchanged = 0;
	...
	firstChar = 0;
#ifdef KANJI
	kanji_flag(newLine, columns);
#endif /* KANJI */
	while (firstChar < columns - 1 &&  newLine[firstChar] == oldLine[firstChar]) {
#ifdef KANJI
		if (kanji_chk[firstChar] == 1) {
			if (newLine[firstChar+1] != oldLine[firstChar+1])
				break;
			firstChar += 2;
			continue;
		}
...
#ifdef	KANJI_XXX
	if (kanji_chk[firstChar] == 2) {
		newLine[firstChar] &= ~0xff;
		newLine[firstChar] |= ' ';
	}
#endif	/* KANJI */
...
#ifdef KANJI
			if (kanji_chk[lastChar] == 2) {
				if (newLine[lastChar-1] != oldLine[lastChar-1])
					break;
				lastChar -= 2;
				continue;
			}
...
#ifdef	KANJI_XXX
		if (kanji_chk[lastChar] == 1) {
			newLine[lastChar] &= ~0xff;
			newLine[lastChar] |= ' ';
		}
#endif	/* KANJI */
		GoTo(lineno, firstChar);
...
#ifdef KANJI
				if (kanji_chk[nLastChar] == 2) {
					if (newLine[nLastChar-1] != oldLine[oLastChar-1])
						break;
					nLastChar -= 2;
					oLastChar -= 2;
					continue;
...
#ifdef	KANJI_XXX
			if (kanji_chk[nLastChar] == 1) {
				newLine[nLastChar] &= ~0xff;
				newLine[nLastChar] |= ' ';
			}
#endif	/* KANJI */
